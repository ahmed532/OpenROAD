                    ...
                    const std::map<std::string, PathAssertion>& path_assertions)
{
  ...
  checkPathAssertions(top_cat, model, path_assertions);
}
...
void Checker::checkPathAssertions(
    dbMarkerCategory* top_cat,
    const UnfoldedModel& model,
    const std::map<std::string, PathAssertion>& path_assertions)
{
  if (path_assertions.empty()) {
    return;
  }
  ...
  for (const auto& [name, assertion] : path_assertions) {
    std::vector<uint32_t> must_touch_ids;
    std::vector<uint32_t> do_not_touch_ids;
    bool valid = true;

    for (const auto& entry : assertion.entries) {
      if (!entry.region_inst) {
        logger_->warn(utl::ODB,
                      473,
                      "Path assertion '{}' skipped: an entry has an unresolved "
                      "region_inst; check earlier resolution warnings",
                      name);
        valid = false;
        break;
      }
      auto ri_it = region_inst_map.find(entry.region_inst);
      if (ri_it == region_inst_map.end()) {
        logger_->warn(
            ...
            "assembled design",
            name,
            entry.region_inst->getChipRegion()->getName());
        valid = false;
        break;
      }
      auto idx_it = region_to_idx.find(ri_it->second);
      if (idx_it == region_to_idx.end()) {
        // Both maps are built from the same model iteration; a pointer that
        // survived the region_inst_map lookup must be in region_to_idx.
    ...
    if (!unreachable.empty()) {
      ...
      // Add source and shape for every unreachable must_touch region so the
      // viewer can highlight exactly which nodes are stranded.
      for (const auto& entry : assertion.entries) {
        if (entry.negated || !entry.region_inst) {
          continue;
        }
        const auto ri_it = region_inst_map.find(entry.region_inst);
        if (ri_it == region_inst_map.end()) {
          continue;
        }
        ...
        if (std::ranges::find(unreachable, idx_it->second)
            != unreachable.end()) {
          marker->addSource(entry.region_inst);
