...
TEST_F(CheckerFixture, test_path_assertion_connected)
{
  ...
  pa.name = "Path1";
  pa.entries = {
      PathAssertionEntry{.region = "", .negated = false, .region_inst = ri1},
      PathAssertionEntry{.region = "", .negated = false, .region_inst = ri2}};
...
TEST_F(CheckerFixture, test_path_assertion_do_not_touch_blocks)
{
  ...
  // Without do_not_touch: r1_fr --(c1)--> r2_bk(inst2) --(clique)-->
  // r2_fr(inst2) --(c2)--> r3_bk Blocking r2_bk: r2_fr is only reachable via
  // r2_bk, so r3_bk becomes unreachable.
  PathAssertion pa;
  pa.name = "Path1";
  pa.entries
      = {PathAssertionEntry{.region = "", .negated = false, .region_inst = ri1},
         PathAssertionEntry{.region = "",
                            .negated = true,
                            .region_inst = ri2_bk},  // do-not-touch
         PathAssertionEntry{
             .region = "", .negated = false, .region_inst = ri3_bk}};
...
TEST_F(CheckerFixture, test_path_assertion_do_not_touch_non_critical)
{
  ...
  pa.entries = {
      ...
      PathAssertionEntry{.region = "",
                         .negated = true,
                         .region_inst = ri2_fr},  // do-not-touch (not on path)
      PathAssertionEntry{
          .region = "", .negated = false, .region_inst = ri2_bk}};

  check({{"Path1", pa}});
  EXPECT_TRUE(getMarkers(path_assertions_category).empty());
}

// 5. No path assertions → no markers (empty map).
...
// 6. Null region_inst → assertion skipped, no crash.
TEST_F(CheckerFixture, test_path_assertion_null_region_inst)
{
  auto* inst1 = dbChipInst::create(top_chip_, chip1_, "inst1");
  inst1->setLoc(Point3D(0, 0, 0));
  inst1->setOrient(dbOrientType3D(dbOrientType::R0, false));
  ...
  pa.entries
      = {PathAssertionEntry{.region = "", .negated = false, .region_inst = ri1},
         PathAssertionEntry{.region = "",
                            .negated = false,
                            .region_inst = nullptr}};  // unresolved entry

  check({{"Path1", pa}});
  // Null region_inst → assertion skipped with a warning, no marker produced.
  EXPECT_TRUE(getMarkers(path_assertions_category).empty());
}
